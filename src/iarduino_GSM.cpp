...
void	iarduino_GSM::pwr(bool f){																															//	Аргументы функции:	f - флаг (0-выключить / 1-включить)
			if(f){_begin();}else{digitalWrite(pinGSMPWR, HIGH);}																							//	Для выключения модуля нужно подать 1 на вывод pinGSMPWR, а включение модуля осуществляется функцией _begin() в которой на вывод pinGSMPWR подаётся 0.
}																																							//	
																																							//	
...
bool	iarduino_GSM::CALLdial(const char* num){																											//	Аргументы функции:	num - строка с номером вызываемого абонента
			...
			if(strlen(num)>10){ if(num[0]!='+'){ if(num[0]!='8'){	runAT( "+",       0, false);	}}}														//	Отправляем символ '+' если [номер не короткий, нет символа '+', номер не начинается с '8'], отводя на получение ответа 0 мс.
